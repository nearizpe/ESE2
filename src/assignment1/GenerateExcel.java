...
public class GenerateExcel {
	...
	public void generateExcel(String path, String pubName, String date, ArrayList<Book> bookList) {
		...
		try {
			...
			Label TitleLabel = new Label(0, 0, "RoyaltyReport", arial15BoldFormatNoBorder); // position of cell and
																							// content of cell
			...
			for (Book temp : bookList) {
				...
				for (AuthorBook authorsTemp : authors) {
					...
					// Number number = new Number (3,i,authorsTemp.getRoyalty().doubleValue()*100);

					String percent = authorsTemp.getRoyalty().doubleValue() * 100 + "%";

					Label number = new Label(3, i, percent);

					totalRoyalty += authorsTemp.getRoyalty().doubleValue() * 100;
				...
				Label totalRoyaltyLabel = new Label(2, i, "Total Royalty", arial10BoldFormatNoBorder);

				String Totalpercent = totalRoyalty + "%";

				Label totalnumber = new Label(3, i, Totalpercent, arial10BoldFormatNoBorder);

				sheet.addCell(totalRoyaltyLabel);
